...
public class Prim {
	...
	public static int[][] generateRandomGraph(int len) {
	...
	public static void main(String args[]) {
		...
		//int[][] adj = generateRandomGraph(200); //generates a random graph with the max number of edges
		//int vertices[] = new int[adj.length];
		int[][] mst = getBlankArr(vertices.length); //will become the adjacency matrix for the MST

		int maxDegree = 7; // <------------------------ Change Max Degree Here!
		...
		while (!areAllTrue(visited)) {
			//minEdge[0] = v1, minEdge[1] = v2
			Integer[] minEdge = getMinEdge(tree, adj);

			if (minEdge[0] >= adj.length || minEdge[1] >= adj.length) { //happens if there is no min edge
				break;
			}
			
			if (degrees[minEdge[0]] == maxDegree || degrees[minEdge[1]] == maxDegree) {
				adj[minEdge[0]][minEdge[1]] = minInt;
				adj[minEdge[1]][minEdge[0]] = minInt;
				continue;
			}

			//reminder: minEdge is (v1, v2, weight, adjMatrix)
			mst = addEdge(minEdge[0], minEdge[1], adj[minEdge[0]][minEdge[1]], mst);

			//removes edge from original adj matrix
			adj[minEdge[0]][minEdge[1]] = minInt;
			adj[minEdge[1]][minEdge[0]] = minInt;

			//adds either v1 or v2 depending on if it has been visited or not
			...
			//Keeps track of the degrees of added vertexes
			degrees[minEdge[0]]++;
			degrees[minEdge[1]]++;
		}
		...
		print("\nMST Total Weight: " + getTotalWeight(mst));
